...
transfer is a 409 rather than a silent replay of the wrong movement. The key namespace is
global, so reusing one that belongs to somebody else is a 409 too.
...
revocable session via a `sid` claim, which is what makes an *access* token revocable, which a blacklist
of refresh tokens structurally cannot do. Replaying a rotated refresh token revokes the
entire token family, so the successor an attacker holds dies along with the one they replayed.
TOTP secrets are envelope-encrypted at rest, so a database dump is not an MFA bypass, under a key
...
read time from rows that already exist, so the endpoint adds no table and no migration. Realized P&L
...
uv run python manage.py seed_instruments --ticks 200   # 57 tickers, with chart history
...
The market only moves while Celery is running, so two more terminals, from `backend/`:
...
`make load` runs a k6 profile against the stack through nginx, fanning out across every seeded
...
the application uses (`open_starter_accounts`, `transfer`, `place_order`) and never through
...
- **One backend image**, four commands: the app replicas, worker, beat, and a one-shot `migrate`
  that runs exactly once per release rather than once per container (ADR-0033, ADR-0036).
- **Releases roll, one replica at a time.** Two app containers sit behind a statically addressed
  nginx upstream, and a deploy replaces them one at a time, so the API and the WebSocket do not drop
  a request (ADR-0043). CI proves it by holding one replica down under continuous traffic. The edge
  is the exception: `web` owns port 80, so it carries its own content derived tag and a backend only
  release leaves it untouched. Migrations have to be expand only, and a CI job refuses the ones that
  are not.
...
map, and it is checked against the schema by nothing, so read it for orientation and the schema for
truth.
...
An account owned by someone else returns 404, never 403, because a 403 would confirm it exists.
...
that sixteen concurrent transfers or trades cannot overdraw an account, oversell a position, or
deadlock each other, that a retried transfer or fill posts once, that a replayed refresh token takes down its
...
486 tests in total, 438 backend and 48 frontend. Coverage is a gate rather than a number in a
commit message: CI fails below 96%. The figure is
**app code only, with branch coverage**, so it excludes the test suite, which is ~100% covered by
construction and was inflating the headline. Measured honestly it is 96.7%, and what remains uncovered
...
deploy/
  compose.yml    the full stack: nginx, two app replicas, worker, beat, Postgres, Redis
  nginx/         the proxy contract, the SPA fallback, the CSP, the JSON error page
  loadtest/      the k6 profile, its token minter, and the recorded results
...
- **A bounded connection pool** over persistent connections, after a load test found that Django's
  ASGI handler opens a thread local connection per in-flight request and exhausted Postgres.
- **Two app replicas behind literal upstream addresses** over a restart gap, because an nginx
  upstream hostname is resolved once at config parse time and cannot be re-resolved during a deploy.
